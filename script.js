...
async function downloadAllAsZip() {
    ...
    try {
        ...
        for (const item of processedOutputs) {
            const squareData = await item.square.blob.arrayBuffer();
            const landscapeData = await item.landscape.blob.arrayBuffer();
            
            zip.file(item.square.name, squareData);
            zip.file(item.landscape.name, landscapeData);
        }
